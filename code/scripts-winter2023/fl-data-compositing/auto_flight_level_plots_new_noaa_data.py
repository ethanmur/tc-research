...
def plot( tc='all', max_v_requirement=40, window=10, method='pmin_tlim', timelim=30, ylims=True):
    ...
    for yeari in folder_input:
        ...
        for dataseti in range( len( fl_list)):
            ...
            fl_data = xr.open_dataset( fl_list[ dataseti])

            time = fl_data.time.values
            ...
            fig = plt.figure( figsize=(24, 22), facecolor='w')
            ...
            ax2.set_ylabel( 'P-3 Height (m)')
            ax2.plot( time, fl_data['HT.d'], c='g', linewidth=.5)
            ax2.grid(True)
