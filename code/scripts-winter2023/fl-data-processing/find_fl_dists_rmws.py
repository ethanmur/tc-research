...
def find_center_dist_rmw( yearval, fileval, tcname, fl_new, float_time):
    ...
    #######
    ## step 1: see if there's valid track data for this case
    #######
    ...
    trackfiles = make_plots.load_flight_level( track_file_path, print_files=False)
    ...
    if np.shape( track_data_temp)[1] == 8:
        print('laura case: add new track columns')
    ...
    #######
    ## step 3: trim down the time ranges on the track data to prevent looking at tracks outside the correct limits!
    #######
    ...
    ########
    ## step 4: find the tc center distance!
    ########
    dists = []
