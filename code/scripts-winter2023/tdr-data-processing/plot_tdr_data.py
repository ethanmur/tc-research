...
# goal: make simple tdr plots of reflectivity and wind speeds for all specified cases.
def plot(tc='all'):
    ...
    for yeari, yearval  in enumerate( yearlist):
        for tci, tcval in enumerate( namelist[yeari]):
            for filei, fileval in enumerate( filelist[yeari][tci][0]):
                ...
                color_map = "bwr"
                xaxis = tdr_data.distance
                wind = tdr_data.Radial_wind[ :, 0:len( xaxis)]
                plt.pcolormesh( xaxis, tdr_data.height, wind, cmap = color_map, vmin = -30, vmax = 30 )
                cbar = plt.colorbar( )
                cbar.ax.set_ylabel( ylabel="TDR Radial Wind (m/s)", fontsize=sz)
                cbar.ax.tick_params(labelsize=sz)
                plt.ylabel( 'Height (Km)')
                ...
                color_map = "bwr"
                xaxis = tdr_data.distance
                wind = tdr_data.Vertical_wind[ :, 0:len( xaxis)]
                divnorm=colors.TwoSlopeNorm(vmin=-10., vcenter=0., vmax=20)
                plt.pcolormesh( xaxis, tdr_data.height, wind, cmap = color_map, norm=divnorm) # vmin = -10, vmax = 20 )
                cbar = plt.colorbar( )
                cbar.ax.set_ylabel( ylabel="TDR Vertical Wind (dBZ)", fontsize=sz)
                cbar.ax.tick_params(labelsize=sz)
                plt.ylabel( 'Height (Km)')
...
# goal: make simple tdr plots of reflectivity and wind speeds for all specified cases.
def plot_ida_tests(tc='all'):
    # get a list of datasets to plot
    path = "/Users/etmu9498/research/data/tdr-one-pass/"
    yearlist = '2021'
    namelist = [['ida']]
    filelist = [[['tdr_combined_ida_210828H1_2113.nc', 'tdr_combined_ida_210828H1_2232.nc', 'tdr_combined_ida_210828H1_2434.nc', 
            'tdr_combined_ida_210828I1_1401.nc', 'tdr_combined_ida_210829H1_1900.nc', 'tdr_combined_ida_210829H1_2016.nc']]]
