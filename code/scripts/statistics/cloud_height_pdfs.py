...
def pdf_all_tc_eyes( tc='all'):
    ...
    for tcname in tcname_list:
        ...
        for dataset in range( len( metadata[ 'dates'] )):
            ...
            new_crl_path = metadata[ 'new_crl_path']
            eyewall_dists = metadata[ 'eyewall_dists'][ dataset]
...
def pdf_one_tc_eye( new_crl_path, new_crl_name, eyewall_dists, title):
    ...
    xaxis_data = crl_data.distance.values
    ...
    H, xaxis_value = cloud_height.find_cloud_heights(new_crl_name, -30, i1, i2, xaxis ='dist', crl_path=new_crl_path)
